    ...
    public partial class frmLookup : ManagedForm
    {
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            if (LookupView.SelectedRows.Count > 0)
            {
                ...
                if (uint.TryParse(txtPrice.Text.Trim(), out uint min)) i.InvMin = min;
                else
                {
                    txt_min_qty.Focus();
                    txtPrice.SelectAll();
                ...
                if (uint.TryParse(txtPrice.Text.Trim(), out uint max)) i.InvMax = max;
                else
                {
                    txt_max_qty.Focus();
                    txtPrice.SelectAll();
                ...
                if (uint.TryParse(txtPrice.Text.Trim(), out uint order)) i.OrderQty = order;
                else
                {
                    txt_min_qty.Focus();
                    txtPrice.SelectAll();
